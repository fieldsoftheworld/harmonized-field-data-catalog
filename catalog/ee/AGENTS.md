...
- Latest edition, stable path: `https://data.source.coop/ftw/harmonized-field-data/ee/latest/ee.parquet`
- One edition: `https://data.source.coop/ftw/harmonized-field-data/ee/year=<year>/<file>.parquet`, e.g. `https://data.source.coop/ftw/harmonized-field-data/ee/year=2024/ee-2024.parquet`
- All editions (hive partitioned): `s3://ftw/harmonized-field-data/ee/year=*/*.parquet` — the S3 form of the same prefix through the Source Cooperative proxy, because `*` needs a listing that plain https does not provide. In DuckDB: `CREATE SECRET sc (TYPE s3, PROVIDER config, ENDPOINT 'data.source.coop', URL_STYLE 'path', REGION 'us-west-2');` then `read_parquet(glob, hive_partitioning = true)` adds the `year` column. No credentials are needed.
- PMTiles for maps: `https://data.source.coop/ftw/harmonized-field-data/ee/year=2024/ee-2024.pmtiles`, layer `ee`; MapLibre styles in `styles/`.
...
- **`hcat:code` is hierarchical.** The first 4/6/8 digits are increasingly specific crop groups; compare prefixes, not equality, to aggregate (see the crop query below). Source crops without a mapping in the converter's HCAT table (`https://fiboa.org/code/ee/ee.csv`) have `NULL`. All 166,508 rows of the 2024 edition carry one.
...
-- 330101 | winter_common_soft_wheat | 40799 | 379424.0
-- 330200 | pasture_meadow_grassland_grass | 74809 | 253540.0
-- 330109 | legumes_harvested_green | 24768 | 146888.0
-- 330106 | winter_rapeseed_rape | 7093 | 69810.0
-- 330102 | peas | 7810 | 66200.0
```
...
FROM read_parquet('https://data.source.coop/ftw/harmonized-field-data/ee/latest/ee.parquet')
WHERE ST_Intersects(geometry, ST_Buffer(ST_Transform(ST_Point(58.5525, 24.9687), 'EPSG:4326', 'EPSG:3301'), 500))
LIMIT 5;
-- id | m2
```
