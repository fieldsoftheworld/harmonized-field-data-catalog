...
- 12 collections hold field *blocks* (reference parcels), not crop fields: `at_block`, `bg`, `de_bb_block`, `de_by_block`, `de_mv`, `de_nds_block`, `de_nrw`, `de_sh`, `de_sl_block`, `de_th`, `lu`, `nl_block`. A block is bounded by permanent features and several farmers and crops can share one, so its rows are not comparable with a crop field's and the two must not be summed. `holds: blocks` in the catalog manifest marks them.
