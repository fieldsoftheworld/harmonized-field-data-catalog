...
def collect_uploads(config: dict[str, str], root: Path = ROOT) -> list[Upload]:
    ...
    for path in sorted(base.rglob("*")):
        # Symlinks are never published: the build links the data files from
        # staging/ into catalog/ so local gates can resolve hrefs, and those
        # bytes go to the bucket through upload_data.py, not from here.
...
def s3_client(config: dict[str, str]):
    """A boto3 S3 client for the configured endpoint and profile.

    Source Cooperative is addressed through its data proxy (``endpoint_url``)
    with temporary credentials that the Source Cooperative CLI hands to the
    AWS SDK via ``credential_process`` on the configured ``profile``. Both keys
    are optional so the script still works against a plain bucket.
    """
    import boto3
...
def remote_index(
    bucket: str, prefix: str, config: dict[str, str] | None = None
