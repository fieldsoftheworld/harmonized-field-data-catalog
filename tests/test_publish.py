...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # Symlinked data files (staging/ linked into catalog/ by the build) are
    # never published from here; upload_data.py sends the real bytes.
    write(root / "staging/roads/year=2024/roads-2024.parquet")
    (root / "catalog/roads/year=2024").mkdir(parents=True)
    (root / "catalog/roads/year=2024/roads-2024.parquet").symlink_to(
        root / "staging/roads/year=2024/roads-2024.parquet"
    )
