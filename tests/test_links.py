...
# Data files are gitignored, so a clean checkout (CI) has the metadata but not
# the bytes. With CI_LIGHT=1 hrefs to these suffixes are counted and skipped,
# and nothing else is. Run without CI_LIGHT locally, with the data in place.
CI_LIGHT = os.environ.get("CI_LIGHT") == "1"
DATA_SUFFIXES = (".parquet", ".pmtiles")
skipped = 0


def skip_in_ci(href: str) -> bool:
...
if skipped:
    print(f"    CI_LIGHT=1: skipped {skipped} data href(s); run without CI_LIGHT"
          " locally with the data in staging to check them")
